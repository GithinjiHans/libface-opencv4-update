 ...
 * @section DESCRIPTION
 *
 * This class is an implementation of Eigenfaces. The image stored is the
 * projection of the faces with the closest match. The relation in recognition
 * is determined by the Eigen value when decomposed.
 *
...
public:
  ...
  /**
   * Performs PCA on the current training data, projects the training faces, and
   * stores them in a DB.
   */
...
void Eigenfaces::EigenfacesPriv::learn(int index, IplImage *newFace) {
  ...
  // Perform PCA
  cvCalcEigenObjects(2, &tempFaces.front(), eigenObjects, CV_EIGOBJ_NO_CALLBACK,
                     0, NULL, &mycrit, pAvgTrainImg, eigenValues);

  cvEigenDecomposite(tempFaces.at(0), 1, eigenObjects, CV_EIGOBJ_NO_CALLBACK,
                     NULL, pAvgTrainImg, projectedFace);

  IplImage *proj = cvCreateImage(size, IPL_DEPTH_8U, 1);
  cvEigenProjection(eigenObjects, 1, CV_EIGOBJ_NO_CALLBACK, NULL, projectedFace,
                    pAvgTrainImg, proj);
...
Eigenfaces::~Eigenfaces() {
  for (std::vector<IplImage *>::iterator it = d->faceImgArr.begin();
       it != d->faceImgArr.end(); ++it)
    cvReleaseImage(&(*it));
...
int Eigenfaces::count() const { return d->faceImgArr.size(); }
...
int Eigenfaces::loadConfig(const string &dir) {
  ...
  CvFileStorage *fileStorage =
      cvOpenFileStorage(d->configFile.data(), 0, CV_STORAGE_READ);
  cout << "opened" << endl;
  if (!fileStorage) {
    if (DEBUG)
  ...
  d->FACE_HEIGHT =
      cvReadIntByName(fileStorage, 0, "FACE_HEIGHT", d->FACE_HEIGHT);
  ...
  for (i = 0; i < nIds; i++) {
    char facename[200];
    sprintf(facename, "person_%d", i);
    d->faceImgArr.push_back(
        (IplImage *)cvReadByName(fileStorage, 0, facename, 0));
  }
...
int Eigenfaces::loadConfig(const map<string, string> &c) {
  ...
  // Not sure what depath and # of channels should be in faceImgArr. Store them
  // in config?
  for (i = 0; i < nIds; i++) {
    char facename[200];
    sprintf(facename, "person_%d", i);
    d->faceImgArr.push_back(LibFaceUtils::stringToImage(
        config[string(facename)], IPL_DEPTH_32F, 1));
  }
...
pair<int, float> Eigenfaces::recognize(IplImage *input) {
  ...
  for (j = 0; j < d->faceImgArr.size(); j++) {
    ...
    cvCalcEigenObjects(2, &tempFaces.front(), eigenObjects,
                       CV_EIGOBJ_NO_CALLBACK, 0, NULL, &mycrit, pAvgTrainImg,
                       eigenValues);

    // This is a simple min distance mechanism for recognition. Perhaps we
    // should check similarity of images.
    ...
    // IplImage *proj = cvCreateImage(cvSize(input->width, input->height),
    // IPL_DEPTH_8U, 1); cvEigenProjection(eigenObjects, nEigens,
    //         CV_EIGOBJ_NO_CALLBACK, NULL, projectedTestFace, pAvgTrainImg,
    //         proj);
...
std::vector<int> Eigenfaces::update(vector<Face> &newFaceArr) {
  ...
  // Our method : If no Id specified (-1), then give the face the next available
  // ID. Now, all ID's, when the DB is read back, were earlier read as the
  // storage index with which the face was stored in the DB. Note that
  // indexIdMap, unlike idCountMap, is only changed when a face with a new ID is
  // added.
  for (i = 0; i < newFaceArr.size(); ++i) {
    if (newFaceArr.at(i).getId() == -1) {
      ...
      // A new face with a new ID is added. So map it's DB storage index with
      // it's ID
      ...
      std::vector<int>::iterator it =
          find(d->indexMap.begin(), d->indexMap.end(), id); // d->indexMap.
      if (it != d->indexMap.end()) {
        ...
        if (DEBUG)
          cout << "Specified ID does not exist in DB, so this is a new face"
               << endl;
